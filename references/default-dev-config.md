...
| frontend | 표현(presentational)과 컨테이너(container) 관심사 분리. 상태 관리와 렌더링 분리. 불필요한 전역 상태 회피. 라우팅과 메뉴 연결: 신규 페이지는 즉시 라우터에 등록하고 메뉴/사이드바의 진입점을 같은 Task에서 추가한다. 라우터·메뉴 배선을 별도 후속 Task로 미루면 orphan page가 발생한다. |
