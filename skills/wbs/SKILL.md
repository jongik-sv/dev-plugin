...
WBS 생성에 들어가기 전에 PRD/TRD가 모호한지 검사한다. **사용자에게 묻지 않고 자율 보강**한다.

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/prd-validate.py validate --target {DOCS_DIR}/PRD.md
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/prd-validate.py validate --target {DOCS_DIR}/TRD.md
```

각 호출의 JSON에서 `issues`가 비어 있지 않으면 다음 절차로 자율 보강한다:

1. issue 목록(placeholder/vague/missing_section)을 읽고 합리적 가정으로 보강 — `{DOCS_DIR}/PRD.md` 또는 TRD에 `## Assumptions (auto-resolved YYYY-MM-DD)` 섹션을 Edit 도구로 append. 템플릿:
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/prd-validate.py assumptions-template
   ```
2. 동일 보강 사항을 `{DOCS_DIR}/decisions.md`에도 append:
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/decision-log.py append \
     --target {DOCS_DIR} \
     --phase prd-resolve \
     --decision-needed "PRD/TRD에 누락·모호 표현: {issue type 목록}" \
     --decision-made "{보강 요약}" \
     --rationale "wbs 생성 전 정합성 확보 — sources: PRD line N, TRD line M" \
     --reversible yes \
     --source "{DOCS_DIR}/PRD.md:{line}"
   ```
3. 보강 후 다시 prd-validate를 호출하여 ok=true 또는 critical issue가 모두 해결된 상태로 진행. 무한 루프 방지를 위해 1회만 재시도하고, 그 후에도 issue가 남으면 사용자에게 한 줄 알림 후 진행한다 (흐름 차단 없음).

### 출력 검증 (8단계 wbs.md 생성 직후)

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/wbs-parse.py {DOCS_DIR}/wbs.md - --dev-config > /tmp/dev-config.json
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/wbs-validate.py validate \
  --wbs {DOCS_DIR}/wbs.md \
  --dev-config-json "$(cat /tmp/dev-config.json)"
```

issue가 있으면 해당 Task만 LLM이 재작성한다 (acceptance 누락 보강, depends 정정 등). 재작성 후 동일 검증을 한 번 더 실행한다. 보강 사항은 `decisions.md`에 `phase=wbs-resolve`로 적재.
...
**0단계 — 공유 계약(shared contract) 사전 분석 (필수, Task 분해 전에 수행)**:

기능을 Task로 쪼개기 **전에** PRD/TRD를 훑어 **"2개 이상 feature가 공유하는 계약"을 모두 식별**한다. "계약"은 DB 엔티티만이 아니라 **shape(구조·스키마)를 공유하는 모든 아티팩트**를 의미한다. 이 판정을 Task 분해 중에 하면 같은 계약이 fullstack 흡수와 독립 선행 Task에 동시에 들어가는 충돌이 발생하고, 공유 계약이 누락되면 feature Task들 사이에 긴 `depends` 체인이 생성되어 병렬성을 잃는다.

**스캔 대상 (4종)**:

| 분석 소스 | 추출 대상 | 예시 |
|----------|----------|------|
| TRD `data-model` 섹션 | DB 테이블·엔티티 | `users`, `organizations` 테이블 |
| TRD `API 설계` 섹션 | 공유 엔드포인트·REST/GraphQL 계약 | `/api/auth/me`, `GET /api/me` 같이 여러 화면이 호출하는 공용 API |
| TRD 타입·DTO·스키마 섹션 (TypeScript interface, Zod, OpenAPI, protobuf 등) | 공유 타입 정의 | `User`, `Session`, `Permission` 타입 |
| TRD 이벤트·메시지 섹션 (도메인 이벤트, 메시지 큐 페이로드) | 공유 이벤트 스키마 | `user.created`, `order.placed` 이벤트 |

각 항목에 대해 "**2개 이상 feature가 참조하는가?**"를 묻는다. 하나라도 "예"면 공유 계약이다.

| 공유 상태 | 배치 |
|----------|------|
| **2개 이상 feature가 참조** (예: `users` 테이블이 로그인·프로필·관리자 3곳 / `/api/auth/me`가 헤더·대시보드·설정 페이지 호출 / `User` 타입이 전역 상태에서 공유) | WP-00 또는 별도 선행 Task로 등록. 이후 모든 feature Task는 이 선행 Task를 `depends`로 연결한다. |
| **1개 feature 전용** (예: `orders` 테이블이 주문 플로우에서만 사용) | 해당 feature Task에 마이그레이션·스키마·타입 정의 포함 (수직 슬라이스) |

**계약 전용(contract-only) Task vs 구현 포함 Task 구분**:

공유 계약을 선행 분리할 때는 **Task 범위를 "계약만 vs 구현까지"로 명시적으로 구분**한다. 이 구분이 없으면 선행 Task가 구현까지 포함해 길어지고 downstream feature Task들이 과도하게 대기한다.

| 유형 | 범위 | category/domain | 예시 |
|------|------|-----------------|------|
| **계약 전용** (contract-only) | 타입 정의 / DDL 파일 / 인터페이스 선언 / OpenAPI·Zod 스키마 / 이벤트 페이로드 정의만. **구현 로직 없음**. | `infrastructure` / `database` 또는 `infra` | `TSK-00-02: users 스키마 + User 타입 정의 (마이그레이션 DDL + TypeScript interface)` |
| **구현 포함** (implementation) | 위 계약 + 실행 로직(마이그레이션 실행, 서비스 구현, 핸들러 등) | `infrastructure` 또는 `development` | `TSK-00-03: users CRUD 서비스 (계약 사용)` |

**왜 구분하나**: 계약 전용 Task는 짧고(수 시간~1일) 단일 도메인이므로 빨리 끝난다. feature Task들은 계약이 확정된 직후부터 **구현 대기 없이** 병렬 진행할 수 있다. 구현 Task(`TSK-00-03`)는 feature와 무관하게 별개 궤도로 진행하거나 필요한 feature만 depends 연결. 이 패턴은 "계약 확정"과 "구현 완료"를 분리해 depends 체인을 짧게 유지한다.

**판정 체크리스트** (각 공유 계약에 대해):
1. 이 계약의 **shape**(필드·타입·엔드포인트·이벤트명)가 확정되면 downstream feature가 mock/stub으로 병렬 진행 가능한가? → **예 → 계약 전용 Task로 분리 권장**
2. 구현까지 있어야 downstream이 동작하는가? → 구현 포함 Task
3. 일부 feature는 계약만 필요, 일부는 실제 구현 필요한가? → 계약 전용 + 구현 포함 2개 Task로 쪼개기
...
| 신호 | 가중치 | 설명 |
|------|-------|------|
| `depends` 개수 ≥ 4 | **+2** | 다중 시스템 교차 — **동시에 "계약 추출 리뷰" 후보로 플래그**. Opus로 올리기 전에 먼저 "이 의존들이 진짜 구현 의존인가, 아니면 shape만 필요한 계약 의존인가?"를 점검하여 공유 계약을 0단계로 되돌려 분리할 수 있는지 확인한다. |
...
`{DOCS_DIR}/wbs.md` 파일을 생성한다. 본 단계에서는 앞 단계까지 확정된 Task 속성을 출력 형식(아래 "출력 형식" 섹션)대로 기록한다. 단, 파일의 **마지막 챕터 `## 의존 그래프`**는 9단계 검증 결과를 반영해야 하므로, 9단계 완료 후 확정된 통계를 반영하여 함께 기록한다(초안을 먼저 써두고 9단계에서 갱신해도 되고, 9단계 실행 후 한 번에 기록해도 된다).

### 9단계: 의존 그래프 검증 (필수)

wbs.md의 Task 목록이 준비되면 `depends` 그래프의 건전성을 검증한다. 이 단계는 **8단계에서 파일을 기록하기 전(또는 직후 갱신) 반드시 수행**한다. 검증은 `scripts/dep-analysis.py --graph-stats`로 자동 계산한다.

```bash
# 모든 pending/completed Task를 포함한 JSON을 입력으로 전달
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/wbs-parse.py" {DOCS_DIR}/wbs.md --tasks-all \
  | python3 "${CLAUDE_PLUGIN_ROOT}/scripts/dep-analysis.py" --graph-stats
```
...
| 필드 | 의미 |
|------|------|
| `max_chain_depth` | 최장 `depends` 체인 길이. **3 초과면 재검토 신호** — 계약 추출·수직 슬라이스 재분해로 단축 가능한지 확인 |
| `fan_in_top` | 가장 많은 Task가 depends로 지목한 상위 Task 목록. **fan-in ≥ 3이면 계약 추출 후보** — 해당 Task를 "계약 전용"과 "구현"으로 분리하면 downstream 병렬도 개선 |
| `diamond_patterns` | `A → B,C → D` 형태의 다이아몬드 의존. B/C가 동시에 D를 기다리면 병렬성이 제한됨. 자주 발생하면 apex Task의 shape만 먼저 확정 |
| `review_candidates` | 위 3개 지표 중 임계값 초과 Task 목록. 각 항목은 `{tsk_id, reason, signal}` |

**판정 기준**:

| 상황 | 대응 |
|------|------|
| `review_candidates`가 비어 있음 | 의존 그래프 양호, 그대로 진행 |
| 후보 존재, 하지만 계약 분리 불가(진짜 구현 의존) | 그대로 진행하되 `## 의존 그래프` 챕터에 근거 명시 |
| 후보 존재하고 계약 분리 가능 | **4단계(Task 분해)로 되돌아가 0단계 공유 계약 분석 보완** — 새 계약 전용 Task 추가, feature Task의 depends 재연결 |

검증 결과는 아래 "출력 형식"의 `## 의존 그래프` 챕터에 **반드시 기록**한다(비어 있어도 "검토 결과 후보 없음"을 명시).
...
> 9단계 검증 결과를 이 섹션에 기록한다. 파일의 **가장 마지막 챕터**로 배치한다(모든 WP·Task 블록 뒤). Mermaid 블록은 `mermaid` 펜스를 사용한다.
...
노드 스타일 규칙:
- 계약 전용 Task는 `style TSK-00-02 fill:#e8f5e9,stroke:#2e7d32`
- 구현 포함 선행 Task는 `style TSK-00-03 fill:#fff3e0,stroke:#e65100`
- 완료 Task(`[xx]`)는 `style TSK-XX fill:#eceff1,stroke:#546e7a`
...
| 항목 | 값 | 임계값 |
|------|-----|--------|
| 최장 체인 깊이 | {max_chain_depth} | 3 초과 시 검토 |
| 전체 Task 수 | {total} | — |
| Fan-in ≥ 3 Task 수 | {fan_in_ge_3_count} | 계약 추출 후보 |
...
| Apex | 분기 | Merge |
|------|------|-------|
| TSK-00-01 | TSK-01-01, TSK-01-02 | TSK-02-01 |
| ... | ... | ... |
...
| TSK-02-07 | fan-in=6 | 분리 | `session` 타입만 공유, 구현은 feature별로 분산 → TSK-00-04(계약 전용) 신설 |
...
- **공유 계약 선행 분리**: 0단계 4종 스캔(data-model / API / 타입·스키마 / 이벤트) 결과 2+ feature가 공유하는 계약이 하나라도 있으면 **계약 전용 Task 또는 구현 포함 Task로 선행 배치**되어야 한다. feature Task들은 해당 선행 Task를 `depends`로 연결한다.
- **의존 그래프 건전성**: 9단계 `dep-analysis.py --graph-stats` 출력 기준으로 `max_chain_depth ≤ 3`이 기본 목표이며, 초과 시 `## 의존 그래프` 챕터에 그 이유와 분리 불가 근거를 반드시 명시한다. `review_candidates`가 비어 있지 않으면 각 후보에 대해 "분리" 또는 "유지+근거"를 기록한다.
- **`## 의존 그래프` 챕터 존재**: 생성된 wbs.md의 마지막 챕터는 `## 의존 그래프`여야 하며, Mermaid 다이어그램 + 통계표 + 리뷰 후보 3개 하위 섹션이 모두 채워져 있어야 한다(후보가 없어도 "후보 없음"을 명시).
