...
tmux가 설치되어 있지 않으면 플랫폼별 설치 방법을 안내하고 중단한다:

> **tmux가 설치되어 있지 않습니다.** 아래 방법으로 설치 후 다시 시도하세요.
>
> | 플랫폼 | 설치 명령 |
> |--------|-----------|
...
> | **Windows** | [psmux](https://github.com/psmux/psmux) 설치 — tmux 호환 Windows 구현 |
>
> 설치 후 `tmux new-session` 으로 세션을 시작하고, 그 안에서 다시 실행하세요.
> tmux 없이 병렬 실행이 필요하면 `/agent-pool`을 사용하세요.

tmux가 설치되어 있지만 세션 밖이면 에러를 출력하고 중단한다:
...
> **tmux 세션 밖에서 실행할 수 없습니다.** `tmux new-session` 으로 세션을 시작하고 다시 시도하세요.
> tmux 없이 병렬 실행이 필요하면 `/agent-pool`을 사용하세요.
...
HEARTBEAT_TIMEOUT=1800  # 초 (30분). worker는 2분 간격으로 하트비트.
