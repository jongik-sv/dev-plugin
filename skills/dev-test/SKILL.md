...
### 단계 1: 단위 테스트
domain에 맞는 단위 테스트를 실행한다:
- backend: `bundle exec rspec` (spec/features, spec/system 제외)
- frontend: `npm run test`
- sidecar: `uv run pytest -m "not e2e"`
- fullstack: backend → frontend → sidecar 순차 실행 (fail-fast: 첫 domain 실패 시 중단)
- database / infra / docs: 해당 domain은 단위 테스트 N/A. "N/A — {domain} domain"으로 기록하고 단계 2로 진행

### 단계 2: E2E 테스트
단위 테스트 결과와 무관하게 E2E 테스트도 실행한다. 파일/명령이 없으면 "N/A"로 기록하고 계속 진행한다:
- backend: `bundle exec rspec spec/features spec/system` (해당 디렉터리가 없으면 N/A)
- frontend: `npm run test:e2e` 실행 — stderr에 "Missing script" 포함 시에만 `npx playwright test` 시도 (테스트 실행 후 실패는 fallback하지 않고 실패로 기록). 둘 다 없으면 N/A
- sidecar: `uv run pytest -m e2e` (e2e 마크 테스트가 없으면 N/A)
- fullstack: backend → frontend → sidecar 순차 실행 (fail-fast: 첫 domain 실패 시 중단)
- database / infra / docs: E2E 테스트 N/A. "N/A — {domain} domain"으로 기록
