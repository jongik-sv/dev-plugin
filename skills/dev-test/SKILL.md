...
UI 키워드 (대소문자 무시, 부분 일치): `button`, `click`, `render`, `form`, `input`, `component`, `modal`, `page`, `screen`, `Playwright`, `Cypress`, `@testing-library`, `화면`, `버튼`, `클릭`, `입력`, `렌더`, `컴포넌트`, `페이지`, `모달`

design.md를 Read 도구로 읽고 본문(코드 블록 포함)에 위 패턴 중 하나라도 있으면 UI 도메인으로 간주한다. 이유: dev-design이 domain을 "default"/"backend"로 잘못 라벨링해도 실제로 UI 작업이면 게이트가 걸려야 함.
...
## Bash 도구 타임아웃
테스트 명령을 Bash 도구로 실행할 때 반드시 `timeout` 파라미터를 설정한다:
- 단위 테스트: `timeout: 360000` (6분)
- E2E 테스트: `timeout: 360000` (6분)
기본값(2분)은 run-test.py 타임아웃(5분)보다 짧아서 Bash가 먼저 프로세스를 죽인다.
...
`${CLAUDE_PLUGIN_ROOT}/references/test-commands.md`의 "E2E 테스트" 섹션을 참조한다.
