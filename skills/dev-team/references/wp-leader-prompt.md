...
에스컬레이션 재할당 방법 (fail_count = 1 또는 2):
```bash
tmux send-keys -t {paneId} '{TEMP_DIR}/task-{task-id}.txt 파일을 Read 도구로 읽고 그 안의 작업을 수행하라. 단, 모든 Phase에 --model {sonnet|opus}를 적용하라.' Enter
```
...
d. 의존 task 차단 해제 → 다음 task 할당 (DONE과 동일 우선순위)
