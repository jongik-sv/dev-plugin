...
`.claude/worktrees/{WP-ID}-prompt.txt`에 아래 내용으로 생성한다.
...
2. **팀리더에게 완료 보고** (시그널 파일, **절대 경로 사용**):
   ...
   **모든 Task 성공 시**:
   ```bash
   cat > {SHARED_SIGNAL_DIR}/{WP-ID}.done.tmp << 'EOF'
   [{WP-ID} 완료]
   ...
   - 특이사항: {있으면 기록, 없으면 "없음"}
   EOF
   mv {SHARED_SIGNAL_DIR}/{WP-ID}.done.tmp {SHARED_SIGNAL_DIR}/{WP-ID}.done
   ```
