...
| `SHARED_SIGNAL_DIR` | `{PROJECT_ROOT}/.claude/worktrees/.signals` | 팀리더↔WP 리더↔팀원 간 공유 시그널 디렉토리 (**절대 경로**) |
...
1. **환경 준비** (worktree + 시그널 디렉토리):
   ...
   # 프로젝트 루트 절대 경로 확정
   PROJECT_ROOT="$(pwd)"
   SHARED_SIGNAL_DIR="${PROJECT_ROOT}/.claude/worktrees/.signals"
...
2. **Task manifest 생성**: `/tmp/team-manifest-{WP-ID}.md`를 team-mode 형식으로 작성한다:
   ...
   ```
   > ⚠️ `signal_dir`은 반드시 **절대 경로**로 지정한다 (`{SHARED_SIGNAL_DIR}` = `{PROJECT_ROOT}/.claude/worktrees/.signals`)
...
5. **WP별 완료 감지 및 조기 머지**:
   ...
   ```bash
   SIGNAL_DIR=".claude/worktrees/.signals"
   WP_ID="{WP-ID}"
...
> ⚠️ 상대 경로(`../.signals/`) 사용 금지 — worktree 내부에서 상대 경로는 `{PROJECT_ROOT}/.claude/worktrees/.signals/`가 아닌 worktree 내부의 다른 위치로 해석될 수 있다.
