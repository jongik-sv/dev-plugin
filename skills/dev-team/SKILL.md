...
| `SHARED_SIGNAL_DIR` | `{TEMP_DIR}/claude-signals/{PROJECT_NAME}` | 팀리더↔WP 리더↔팀원 간 공유 시그널 디렉토리 (**절대 경로**) |
...
1. **환경 준비** (worktree + 시그널 디렉토리):
   ...
   SHARED_SIGNAL_DIR="${TEMP_DIR}/claude-signals/${PROJECT_NAME}"
...
2. **Task manifest 생성**: `{TEMP_DIR}/team-manifest-{WP-ID}.md`를 team-mode 형식으로 작성한다:
   ```markdown
   # Configuration
   - team_size: {TEAM_SIZE}
   - window_name: {WP-ID}
   ...
   ```
   > ⚠️ `signal_dir`은 반드시 **절대 경로**로 지정한다 (`{SHARED_SIGNAL_DIR}` = `{TEMP_DIR}/claude-signals/{PROJECT_NAME}`)
...
4. **WP 리더 spawn**:
   ...
   tmux new-window -t "${SESSION}" -n "{WP-ID}" .claude/worktrees/{WP-ID}-run.sh
   # window 이름 보호 (프로세스명으로 덮어씌워지는 것 방지)
   tmux set-option -w -t "${SESSION}:{WP-ID}" automatic-rename off
   tmux set-option -w -t "${SESSION}:{WP-ID}" allow-rename off
   tmux set-option -w -t "${SESSION}:{WP-ID}" pane-border-status top
   tmux set-option -w -t "${SESSION}:{WP-ID}" pane-border-format " #{pane_index}: #{@label} "
   ```

5. **WP별 완료 감지 및 조기 머지**:
   팀리더는 각 WP의 시그널 파일을 Bash `run_in_background`로 감시한다:
   ```bash
   SIGNAL_DIR="${TEMP_DIR}/claude-signals/{PROJECT_NAME}"
   WP_ID="{WP-ID}"
...
```bash
for PANE_ID in $(tmux list-panes -t "${SESSION}:{WP-ID}" -F '#{pane_id}'); do
  tmux send-keys -t "${PANE_ID}" Escape 2>/dev/null
  sleep 1
  tmux send-keys -t "${PANE_ID}" '/exit' Enter 2>/dev/null
done
sleep 3
tmux kill-window -t "${SESSION}:{WP-ID}" 2>/dev/null
```
...
3. 머지 후 충돌 여부 확인
   ...
   - 충돌 없으면: 다음 브랜치 머지 진행
4. 모든 머지 완료 후 정리:
   - 시그널 디렉토리 정리: `rm -rf ${TEMP_DIR}/claude-signals/{PROJECT_NAME}`
