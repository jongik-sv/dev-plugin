...
각 Task에 대해 `{TEMP_DIR}/task-{TSK-ID}.txt`를 생성한다.
...
1. **환경 준비** (worktree + 시그널 디렉토리):
   ```bash
   # 프로젝트 이름 및 시그널 디렉토리 확정 (서브프로젝트 suffix 포함)
   PROJECT_NAME="$(basename "$(pwd)")"
   SHARED_SIGNAL_DIR="${TEMP_DIR}/claude-signals/${PROJECT_NAME}${WINDOW_SUFFIX}"

   # worktree 생성 (서브프로젝트 있으면 브랜치/경로에 suffix 포함)
   WT_NAME="{WP-ID}${WINDOW_SUFFIX}"
   git worktree add .claude/worktrees/${WT_NAME} -b dev/${WT_NAME}

   # 시그널 디렉토리 생성
   mkdir -p "${SHARED_SIGNAL_DIR}"
   ```

2. **Task manifest 생성**: `{TEMP_DIR}/team-manifest-{WT_NAME}.md`를 team-mode 형식으로 작성한다
   (`{WT_NAME}` = `{WP-ID}{WINDOW_SUFFIX}`, 예: `WP-04` 또는 `WP-04-p1`):
   ```markdown
   # Configuration
   - team_size: {TEAM_SIZE}
   - window_name: {WT_NAME}
   ...
   - prompt_file: {TEMP_DIR}/task-{TSK-ID}.txt
   ```
   > ⚠️ `signal_dir`은 반드시 **절대 경로**로 지정한다 (`{SHARED_SIGNAL_DIR}` = `{TEMP_DIR}/claude-signals/{PROJECT_NAME}{WINDOW_SUFFIX}`)
   > ⚠️ `docs_dir`은 상대 경로 (`docs` 또는 `docs/{SUBPROJECT}`). worktree 내부에서도 동일하게 해석된다.

3. **WP 리더 프롬프트 생성**: `.claude/worktrees/{WT_NAME}-prompt.txt`를 아래 "WP 리더 프롬프트" 섹션 내용으로 작성한다

4. **WP 리더 spawn**:
   ```bash
   # 래퍼 스크립트 생성
   cat > .claude/worktrees/${WT_NAME}-run.sh << SCRIPT_EOF
   #!/bin/bash
   cd "\$(dirname "\$0")/${WT_NAME}"
   exec claude --dangerously-skip-permissions "\$(<../"${WT_NAME}-prompt.txt")"
   SCRIPT_EOF
   chmod +x .claude/worktrees/${WT_NAME}-run.sh

   # tmux 창으로 실행 (--workdir로 worktree 경로 지정)
   tmux new-window -t "${SESSION}" -n "${WT_NAME}" .claude/worktrees/${WT_NAME}-run.sh
...
5. **WP별 완료 감지 및 조기 머지**:
   ...
   ```bash
   SIGNAL_DIR="${TEMP_DIR}/claude-signals/${PROJECT_NAME}${WINDOW_SUFFIX}"
   WT_NAME="{WP-ID}${WINDOW_SUFFIX}"
   while [ ! -f "${SIGNAL_DIR}/${WT_NAME}.done" ]; do sleep 15; done
   echo "WP_DONE:${WT_NAME}"
   cat "${SIGNAL_DIR}/${WT_NAME}.done"
   ```
