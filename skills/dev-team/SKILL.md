...
모든 시그널 파일은 `signal-helper.py`로 원자적으로 생성한다:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/signal-helper.py done {TSK-ID} {SHARED_SIGNAL_DIR} "메시지"
```
...
2. **셋업 스크립트 실행**:
   ```bash
   python3 "${PLUGIN_ROOT}/scripts/wp-setup.py" "${TEMP_DIR}/wp-setup-config.json"
   ```
...
```
6. 충돌 발생 시: 사용자에게 보고하고 수동 해결 요청. 60초 후 재확인 (최대 3회). 3회 초과 시 `git merge --abort`로 해당 WP 머지 건너뛰기
7. worktree + 브랜치 정리:
```bash
