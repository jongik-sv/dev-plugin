...
task 파일(JSON)을 dep-analysis.py로 분석한다:

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/dep-analysis.py {task-file-path}
```

JSON 출력에서 레벨별 실행 순서와 순환 의존 여부를 확인한다:
```json
{
  "levels": {"0": ["task-1", "task-3"], "1": ["task-2"]},
  "circular": [],
  "pending": 3
}
```

`circular`가 비어있지 않으면 에러를 보고하고 중단한다.

> Markdown 형식 task 파일의 경우, JSON으로 변환 후 파이프하거나 수동으로 레벨을 산출한다.
