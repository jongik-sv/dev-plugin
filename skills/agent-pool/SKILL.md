...
| `SIGNAL_DIR` | `{TEMP_DIR}/agent-pool-signals-{YYYYMMDD-HHmmss}` | 시그널 파일 디렉토리 (타임스탬프로 유니크) |
...
- `$ARGUMENTS`에 파일 경로가 있으면 → **1-B. Task 파일 파싱**
- 파일 경로가 없으면 → **1-A. 대화형 Task 수집**

### Task 스키마 (1-A / 1-B 공통)

각 task는 3개 필드를 가진다 — 1-A 자동 생성과 1-B 파일 입력 모두 동일.

| 필드 | 타입 | 설명 |
|------|------|------|
| `id` | string | `task-1`, `task-2`, ... 순차 부여 |
| `prompt` | string | 서브에이전트에 전달할 작업 본문 |
| `depends` | string[] | 선행 task id 리스트 (없으면 `[]`) |

**JSON 표준형** (1-A 생성물 + 1-B JSON 파일이 모두 사용):
```json
{
  "tasks": [
    { "id": "task-1", "prompt": "이 작업을 수행하라...", "depends": [] },
    { "id": "task-2", "prompt": "이 작업을 수행하라...", "depends": ["task-1"] }
  ]
}
```
...
응답을 파싱하여 위 JSON 표준형을 `{TEMP_DIR}/agent-pool-tasks.json`에 자동 생성한다. 의존성 미언급 시 모든 task의 `depends`를 `[]`(전부 병렬). 생성 파일을 사용자에게 보여주고 확인 후 진행.

### 1-B. Task 파일 파싱

`$ARGUMENTS` 경로의 파일을 Read 도구로 읽는다. 형식은 JSON(위 표준형)과 Markdown 둘 다 지원.

**Markdown 형식** (스키마는 동일, 표기만 다름):
```markdown
### task-1
- depends: (none)
- prompt: |
...
파싱 후 각 task에서 `id`, `prompt`, `depends`를 추출한다.

## 2. 의존성 분석

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/dep-analysis.py {task-file-path}
```

출력 JSON의 `levels`로 실행 순서를 결정한다. `circular`가 비어있지 않으면 에러 보고 후 중단. Markdown 형식 task 파일은 JSON으로 변환 후 전달한다.

## 3. 풀 실행

### 시그널 디렉토리 생성

`SIGNAL_DIR`을 타임스탬프로 유니크하게 결정한다:
```bash
SIGNAL_DIR="{TEMP_DIR}/agent-pool-signals-$(date +%Y%m%d-%H%M%S)-$$"
mkdir -p "${SIGNAL_DIR}"
```

> 시그널 파일 생성/확인은 `signal-helper.py`로 간소화할 수 있다:
> `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/signal-helper.py done {task-id} {SIGNAL_DIR} "message"`
> ⚠️ signal-helper.py는 시그널 파일 내용을 최대 50줄로 자동 절삭한다. 대량 출력(스택 트레이스 등)은 별도 파일에 기록하라.
...
출력이 `DONE:{task-id}`이면 성공, `FAILED:{task-id}`이면 실패.
...
1개 완료 → 1개 투입으로 `POOL_SIZE` 슬롯을 항상 채운다 (배치 대기 금지).
