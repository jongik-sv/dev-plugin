...
### 3. Verification Gate (design.ok 직전 필수)

상태 전이 직전에 phase 종료 verification을 실행한다. 차단 시 `design.ok` 호출하지 않는다.

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/verify-phase.py \
  --phase design \
  --target {ARTIFACT_DIR} \
  > /tmp/verify-design-{ID}.json
```

종료 코드 0이면 통과, 1이면 차단. 차단 시 footer.checks의 실패 항목(필수 섹션 누락, 체크박스 0개 등)을 동일 서브에이전트에 재지시하여 design.md 수정 후 재실행.

상세 프로토콜: `${CLAUDE_PLUGIN_ROOT}/references/verification-protocol.md`.

### 4. 상태 전이

verification 통과 시에만 `design.ok` 이벤트를 발생시킨다. 설계 실패는 **DFA 이벤트가 아니다** — 서브에이전트 크래시/사용자 중단/I-O 에러 등 인프라 예외로 취급하며, 상태는 `[ ]`에 머문 채 그대로 사용자에게 에러를 보고하고 종료한다.

**(A) SOURCE=wbs** — Bash 도구로 실행:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/wbs-transition.py {DOCS_DIR}/wbs.md {TSK-ID} design.ok \
  --verification /tmp/verify-design-{ID}.json
```
...
> 상태 전이 규칙은 `${CLAUDE_PLUGIN_ROOT}/references/state-machine.json`에 정의되어 있으며 두 모드가 동일한 DFA를 공유한다. 스크립트는 사이드카 `state.json`을 원천으로 갱신하고(verification footer를 phase_history 항목에 합성) wbs.md status 줄(wbs 모드)을 동기화한다.

전이 스크립트 자체가 에러(파일 없음 등)를 반환하면 사용자에게 보고 후 종료.

### 5. 완료 보고
- 성공: 생성된 design.md 경로와 설계 요약 출력. state.json의 `status`는 `[dd]`, `last.event`는 `design.ok`, 새 phase_history 항목에 verification footer 합성됨.
- 실패 (인프라 예외): 실패 원인을 그대로 사용자에게 보고. 상태는 `[ ]` 유지. 호출자(`/dev`, `/feat` 오케스트레이터)는 서브에이전트의 에러 출력과 미생성된 design.md로 실패를 감지한다.

### 자율 결정 기록

설계 중 비자명한 자율 결정(라이브러리 선택, 모호 요구의 가정 채택, 의존성 해석 등)을 한 경우 `${CLAUDE_PLUGIN_ROOT}/scripts/decision-log.py append --target {ARTIFACT_DIR} --phase design ...`로 `decisions.md`에 적재한다. 판별 휴리스틱: `${CLAUDE_PLUGIN_ROOT}/references/decisions-template.md` 참조.
