...
## TDD 순서 (반드시 준수)
1. design.md의 QA 체크리스트 기반으로 테스트를 먼저 작성
2. 테스트가 실패하는 것을 확인 (Red)
3. 테스트를 통과하는 최소한의 코드 구현 (Green)
4. 테스트를 실행하여 전부 통과 확인

## domain별 테스트 프레임워크 (단위 테스트만 — E2E 제외)
- backend: RSpec (`bundle exec rspec --exclude-pattern "spec/features/**/*,spec/system/**/*"`)
- frontend: Vitest (`npm run test`)
- sidecar: pytest (`uv run pytest -m "not e2e"`)
