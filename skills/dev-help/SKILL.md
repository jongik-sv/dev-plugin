...
| `/dev-team` | WP 단위 팀 병렬 개발 (tmux+워크트리). `--sequential`로 워크트리 없이 WP 순차 실행도 지원 | `/dev-team WP-04` 또는 `/dev-team --sequential WP-01 WP-02` |
...
/dev-team WP-04 WP-05                 # 복수 WP 동시 실행 (병렬+워크트리)
/dev-team p1 WP-01 WP-02              # 서브프로젝트에서 복수 WP 동시 실행
/dev-team                             # 실행 가능 WP 자동 선정 (docs/)
/dev-team p1                          # docs/p1/에서 실행 가능 WP 자동 선정
/dev-team WP-04 --team-size 5         # 팀원 수 변경 (기본: 3명)
/dev-team --sequential WP-01 WP-02    # 순차 모드: WP 하나씩, 워크트리 없음
/dev-team --sequential                # 순차 모드 + WP 자동 선정
/dev-team --seq --team-size 5 WP-01   # 순차 모드 + 팀원 5명
/dev-team p1 WP-01 순차실행           # NL 키워드로 순차 모드 (한국어)
/dev-team monitor-v4 순차             # "순차" 단독 토큰도 인식
```

> 💡 **NL 키워드 지원**: `/dev-team`은 `--sequential` 플래그 외에도 인자 어디에 있든 `순차`/`순차실행`/`순차모드`/`순차모드로`/`sequential`/`seq` 토큰을 만나면 순차 모드를 활성화한다. `/dev-team monitor-v4 순차실행 해`처럼 자연어로 불러도 작동한다.
...
**`--sequential` 모드** (워크트리 없이 WP 순차 실행):
- WP 간 순차 + WP 내부 병렬(팀원 pane) — 둘의 중간 지점
- 워크트리 없음 → 머지 절차 없음 → 현재 브랜치에 직접 커밋
- **선택 기준**: 동시 다WP 병렬은 부담스럽지만 WP 내부 팀원 병렬은 살리고 싶을 때. 리소스(비용·메모리) 절약 + WP 내부 병렬 속도 유지.

| 모드 | WP 간 | WP 내부 | 워크트리 | 비용 |
|------|-------|---------|---------|------|
| `/dev-team` (기본) | 병렬 | 병렬 | O | 높음 |
| `/dev-team --sequential` | **순차** | 병렬 | **없음** | 중간 |
| `/dev-seq` | 순차 | 순차 1명 | 없음 | 낮음 |
